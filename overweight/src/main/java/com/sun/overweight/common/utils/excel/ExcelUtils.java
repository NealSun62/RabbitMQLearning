package com.sun.overweight.common.utils.excel;
...
//        }
    ...
    public static void export(HttpServletResponse response, File file, String fileName, String sheetName,
                              List<List<Object>> sheetDataList, Map<Integer, List<String>> selectMap) {
        // 整个 Excel 表格 book 对象
    ...
    public static void export(HttpServletResponse response, File file, String fileName, String sheetName,
                              List<List<Object>> sheetDataList, Map<Integer, List<String>> selectMap, String templeFilePath) throws IOException {
        // 整个 Excel 表格 book 对象
        SXSSFWorkbook book = null;
        InputStream is = null;
        try {
            if (StringUtils.isNotBlank(templeFilePath)) {
                ClassPathResource classPathResource = new ClassPathResource(templeFilePath);
                is = classPathResource.getInputStream();
                XSSFWorkbook workbook = new XSSFWorkbook(is);
                book = new SXSSFWorkbook(workbook, 100);
            } else {
                new SXSSFWorkbook();
            }
        } catch (Exception e) {
    ...
    private static int setCellValue(Cell cell, Object o, CellStyle style) {
        ...
        // 如果是BigDecimal，则默认6位小数
        if (o instanceof BigDecimal) {
            cell.setCellType(CellType.NUMERIC);
            cell.setCellValue(((BigDecimal) o).setScale(6, RoundingMode.HALF_UP).doubleValue());
            return CELL_OTHER;
        }
        // 如果是Date数据，则显示格式化数据
